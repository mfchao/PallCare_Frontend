...
export default class LetterConcept<T> {
  ...
  async createLetter(from: ObjectId, to: ObjectId[], content: T, responseEnabled: boolean, response: LetterResponseDoc<T>[] = [], send: boolean = false, show: boolean = true) {
  ...
  async updateLetterContent(_id: ObjectId, content: T) {
  ...
  async createLetterResponse(letter: ObjectId, responsefrom: ObjectId, content: T | null , response: LetterResponseDoc<T>[] = []) {
